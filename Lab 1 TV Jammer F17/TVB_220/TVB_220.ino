...
void xmitCodeElement(uint16_t ontime, uint16_t offtime, uint8_t freq )
{
  uint16_t elapsedTime = 0;
  TCNT2 = 0;
  /* 
   * 220_TODO: edit the if statement below to check if the frequency is nonzero.
   * If the frequency is zero, that means the IR LED should just stay on for the specified
   * ontime. If the frequency is nonzero, that means we're going to have to pulse the LED
   * at the given carrier frequency for the ontime.
   */
  if (freq != 0 ) /* 220_TODO: re-write this line as required */  {
    // This is the case where the frequency is nonzero, so we use PWM at the given
    // carrier frequency
     ...
     while(elapsedTime < ontime){
      ...
      delay(1000/(2*freq));
      elapsedTime += (1000/freq);
      }
    }
    // 220_TODO: add code after this comment to set the IR LED pin to output
    pinMode(LED, OUTPUT);
...
void setup()   {
  ...
  // 220_TODO: write a digital low to the IR LED pin for initialization
  
  digitalWrite(LED, LOW);
  digitalWrite(DBG, LOW);     // debug
  pinMode(LED, OUTPUT);
  // 220_TODO: configure the IR LED pin as an output
  
  pinMode(DBG, OUTPUT);       // debug

  // 220_TODO: configure the pushbutton trigger pin as an input
...
void adjustPWMFreq() {
  ...
  // Output pin should already be set by calling function
...
Start_transmission:
  ...
  if (digitalRead(REGIONSWITCH)) {
    region = NA;
    if (QUICK == 0) num_codes = num_NAcodes;
    else num_codes = 0;
  }
  else {
    region = EU;
  ...
  if (QUICK) {
    ...
    // print out the code # 20
    DEBUGP(putstring("\n\r\n\rCode #: ");
    putnum_ud(20));
    
    data_ptr = (PGM_P)pgm_read_word(NApowerCodes+20);
...
//      xmitCodeElement(ontime, offtime, (freq!=0));
      xmitCodeElement(ontime, offtime, freq);
    }
    sei();
...
void loop() {
  ...
  /* 
   * 220_TODO: From the pre-lab: determine if the button is pressed
   *  If the button is pressed, call sendAllCodes() to begin transmission
   *  If not, call delay_ten_us(3000) to delay 30 ms until next read
   */
}
